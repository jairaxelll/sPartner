...
struct ContentView: View {
    ...
    var cardViews: [CardView] = {
        var views = [CardView]()
        for index in 0..<2 {
            views.append(CardView(sPartner: sPartnerData[index]))
        }
        return views
    }()
    ...
    var body: some View {
        ...
        .alert(isPresented: $showAlert) {
            Alert(
                title: Text("SUCCESS"),
                message: Text("-Directly move to chat-"),
                dismissButton: .default(Text("Under development")))
        }
    }
    ...
    private func moveCards() {
        ...
        let sPartner = sPartnerData[lastCardIndex % sPartnerData.count]
        
        let newCardView = CardView(sPartner: sPartner)
